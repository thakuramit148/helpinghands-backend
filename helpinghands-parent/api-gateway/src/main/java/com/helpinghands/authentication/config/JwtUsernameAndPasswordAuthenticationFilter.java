...
import com.helpinghands.authentication.model.LoginModel;
import com.helpinghands.authentication.model.ProfileForLoginState;
...
public class JwtUsernameAndPasswordAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ProfileService.tableName = creds.getUsername().split("<>")[1];
			creds.setUsername(creds.getUsername().split("<>")[0]);
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		response.getWriter().write(mapper.writerWithDefaultPrettyPrinter().writeValueAsString(new ProfileForLoginState(
				profileModel.getId(), profileModel.getUsername(), profileModel.getRole(), profileModel.isActive())));
	}
}
